...
<c:choose>
	<c:when test="${subq == true}">
		<div class="floatInputBasic travelAdjust" style="float: left">
				...
				<div>
					<spring:bind path="${vari}">
						<div class="checkbox">
							<label class="check"> <form:checkbox id="${id}"
									path="${status.expression}" class="logCheck" /> <span
								class="checkbox-replacement-helper"></span> <spring:message
									code="${lbl}" arguments="${lblarg}" />
							</label>
						</div>
						<c:if test="${status.error}">
							<div class="appError">
					...
					<div class="revItemLeft">
						...
						<div id="LABEL${id}">
							<span><spring:bind path="${vari}">
									<div class="checkbox">
										<label class="check"> <form:checkbox id="${id}hidden" disabled="true"
												path="${status.expression}" class="logCheck" />
											<span class="checkbox-replacement-helper"></span> <spring:message
												code="${lbl}" />
										</label>
									</div>
								</spring:bind></span>
...
$(function() {
	...
	$('#BTSV${id}').click(
			function() {
				...
				if(current){
					$('#${id}hidden').prop('checked', true);
				} else {
					$('#${id}hidden').prop('checked', false);
				}
