...
<div class="breadcrumb">
	<h3>Ledger Primary Validation</h3>					
	<a href="homePage">Home</a> » <a href="#">Ledger Primary Validation</a>
</div>	
<div class="col-md-12" >
		...
		<table id="table" 
			...
			<tbody>
				<c:forEach var="ledger" items="${ledgerList}">
					<tr>
					<td><input name="32-ck"
							id="32-ck" value="${ledger.ledger_id}" type="checkbox"
							onchange="addPro(this)"></td></td>
						...
						<a href="#"
							onclick="approveLedger('${ledger.ledger_id}')" title="Approve"><img
								src='${approveImg}' style="width: 20px;" /></a> <a href="#"
...
$(function() {		
	...
	function viewLedger(id){
		window.location.assign('<c:url value="viewLedger"/>?id='+id+'&flag='+2);
	}
	...
	function approveLedger(id){
		window.location.assign('<c:url value="approveLedger"/>?id='+id + "&primaryApproval="+true);
	}
	
	function rejectLedger(id){
		if (confirm("Are you sure you want to Reject record?") == true) {
			window.location.assign('<c:url value="rejectLedger"/>?id='+ id + "&rejectApproval="+true);
		    } 
	}
