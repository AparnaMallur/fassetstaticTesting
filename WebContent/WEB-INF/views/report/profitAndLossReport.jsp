...
<div class="breadcrumb">
	<h3>Profit And Loss AccountAA</h3>
	<a href="homePage">Home</a> » <a href="#">Profit And Loss Account</a>
</div>
<div class="col-md-12 wideform">
	<div class="fassetForm">
		<form:form id="verticalForm" action="profitAndLossReport" method="post" commandName="form" onsubmit="return checkdata();">
			<div class="row">
...
<script type="text/javascript">
	...
	function getList(groupId) {
		<c:forEach var = "group" items = "${accountGroupList}">
		var id = <c:out value = "${group.group_Id}"/>
		if (id == groupId) {
			$('#subgroupId').find('option').remove();
			$('#subgroupId').append($('<option>', {
				value : 0,
				text : 'Select Subgroup Name'
			}));
			<c:forEach var = "subgroup" items = "${group.account_sub_group}">
			console
					.log("id :  ${subgroup.subgroup_Id}   name : ${subgroup.subgroup_name}");
			$('#subgroupId').append($('<option>', {
				value : '${subgroup.subgroup_Id}',
				text : '${subgroup.subgroup_name}',
			}));
			</c:forEach>
		}
		</c:forEach>
