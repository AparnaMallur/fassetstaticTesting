	...
	<div class="fassetForm">
		<form:form action="payrollAutoJV" method="post" commandName="payrollAutoJV" onsubmit="return save();">
			...
			<%-- 	<c:if test='${payrollAutoJV.payroll_id!=Null}'>
			    <form:input path="accountYearId" hidden="hidden" />
			    </c:if> --%>
			<div class="row">
					<c:if test='${payrollAutoJV.payroll_id!=Null}'>
						<div class="row">
			...
			<div class="row">	
				<div class="col-md-4 control-label"><label>Working Day<span>*</span></label></div>		
					<div class="col-md-8">
						<form:input path="days" class="logInput" id = "wdays"   placeholder="Working Days" maxlength="2"  />
					</div>
			</div>
		...
		<tbody>
			<%-- <c:forEach var = "payrollemployeedetail" items = "${payrollemployeedetail}">
				<tr>
					<td>
						<a href = '#' onclick = 'deleteRow(this,"+id+")'>
						<img src='${deleteImg}' style = 'width: 20px;'/></a> 
						<a href="#" onclick="editpayroll(${payrollemployeedetail.employeeDetails_id})">
				...
				<table class="table table-bordered table-striped">
					...
					<c:if test="${payrollAutoJV!=null}">
						<tr>
							...
							<td class="tright"><input id="grossSalary" class="logInput" placeholder="Gross Salary"  onkeypress="return isNumber(event)" maxlength="10"/> </td>
							
							<td class="tright"><input id="pfEmployeeContribution" class="logInput" placeholder="PF EmployeeContribution"   onchange="calculaterate1(this.value,1)" onkeypress="return isNumber(event)" maxlength="10"/> </td>
							
							<td class="tright"><input id="eSICEmployeeContribution" class="logInput" placeholder="ESIC Employee Contribution"  onchange="calculaterate1(this.value,2)" onkeypress="return isNumber(event)" maxlength="10"/> </td>
							
							<td class="tright"><input id="professionTax" class="logInput" placeholder="Profession Tax" onchange="calculaterate1(this.value,3)" onkeypress="return isNumber(event)" maxlength="10"/> </td>
							
							<td class="tright"><input id="lWF" class="logInput" placeholder="LWF"  onkeypress="return isNumber(event)"  onchange="calculaterate1(this.value,4)" maxlength="10"/> </td>
							
							<td class="tright"><input id="tDS" class="logInput" placeholder="TDS"  onkeypress="return isNumber(event)" onchange="calculaterate1(this.value,5)" maxlength="10"/> </td>
							
							<td class="tright"><input id="otherDeductions" class="logInput" placeholder="Other Deductions"  onchange="calculaterate1(this.value,6)"  onkeypress="return isNumber(event)" maxlength="10"/> </td>
							
							<td class="tright"><input id="advanceAdjustment" class="logInput" placeholder="Advance Adjustment"  onchange="calculaterate1(this.value,7)"  onkeypress="return isNumber(event)" maxlength="10"/> </td>
							...
							<td class="tright"><input id="netSalary" class="logInput" placeholder="netSalary"  onkeypress="return isNumber(event)" onchange="calculaterate1(this.value,7)"  readonly="true" maxlength="10"/> </td>
							
							<td class="tright"><input id="pfEmployerContribution" class="logInput" placeholder="PF EmployerContribution" onkeypress="return isNumber(event)" onchange="calculaterate1(this.value,8)" maxlength="10"/> </td>
							
							<td class="tright"><input id="eSICEmployerContribution" class="logInput" placeholder="ESIC Employer Contribution"  onkeypress="return isNumber(event)" onchange="calculaterate1(this.value,9)"  maxlength="10"/> </td>
							
							<td class="tright"><input type="text" id="pFAdminCharges" class="logInput" placeholder="PF Admin Charges"  onkeypress="return isNumber(event)" onchange="calculaterate1(this.value,10)" maxlength="10" /> </td>
...
function isValidDate(dateStr) {
	 alert("in isValidDate is :"+dateStr);
...
var administrativeExpense="Administrative expense";
...
function setMyData(){
	<c:forEach var = "ee" items = "${payrollemployeedetail}">

		empId="${ee.code}";
 		
		id = "${ee.code}"; 
		 employeeName = "${ee.name}";
		 PAN = "${ee.pan}";
		 basicSalary ="${ee.basicSalary}";
		 DA =  "${ee.DA}";
		 totaldays = "${ee.totaldays}";
		 conveyanceAllowance ="${ee.conveyanceAllowance}";
		 otherAllowances = "${ee.otherAllowances}";
		 grossSalary =" ${ee.grossSalary}";
		 pfEmployeeContribution ="${ee.pfEmployeeContribution}";
		 eSICEmployeeContribution =" ${ee.eSICEmployeeContribution}";
		 professionTax ="${ee.professionTax}";
		 lWF =" ${ee.lWF}";
		 tDS = "${ee.tDS}";
		 otherDeductions ="${ee.otherDeductions}";
		 advanceAdjustment = "${ee.advanceAdjustment}";
		 totalDeductions =" ${ee.totalDeductions}";
		 netSalary = "${ee.netSalary}";
		 pfEmployerContribution = "${ee.pfEmployerContribution}";
		 eSICDrAmount ="${ee.eSICEmployerContribution}";
		 pFAdminCharges ="${ee.pFAdminCharges}";
		 totalBasicSalary = parseFloat(totalBasicSalary) + parseFloat(basicSalary);
			...
			totalESICEmployerContribution = parseFloat(totalESICEmployerContribution) + parseFloat(eSICDrAmount);
			totalPFAdminCharges = parseFloat(totalPFAdminCharges) + parseFloat(pFAdminCharges);
			addRow(id,empId,employeeName,PAN,totaldays,basicSalary,DA,conveyanceAllowance,otherAllowances,grossSalary,pfEmployeeContribution,eSICEmployeeContribution,professionTax,lWF,tDS,otherDeductions,advanceAdjustment,totalDeductions,netSalary,pfEmployerContribution,eSICDrAmount,pFAdminCharges);
...
function addRow(id,empId,employeeName,PAN,totaldays,basicSalary,DA,conveyanceAllowance,otherAllowances,grossSalary,pfEmployeeContribution,eSICEmployeeContribution,professionTax,lWF,tDS,otherDeductions,
		advanceAdjustment,totalDeductions,netSalary,pfEmployerContribution,eSICDrAmount,pFAdminCharges){
	empDetails.push({"id":id,
		...
		"eSICEmployerContribution":eSICDrAmount,
