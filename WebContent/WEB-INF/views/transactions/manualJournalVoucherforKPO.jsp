...
<c:if test="${successMsg != null}">
		...
		<div class="fassetForm">
			...
			<c:choose>
						<c:when test="${((role == '2') || (role == '3') ||(role == '4'))}">	
							<div class="row">			
								...
								<div class = "col-md-10">
									<form:select path="company_id" id="company_id" class="logInput" onchange = "getSubledgers(this.value)">
			...
			<div class="row">	
				...
				<div class="row">
					...
					<div class="col-md-5">
						<form:select path="subledger_id1" class="logInput" id="select_dr_id" >
							<form:option value="0" label="Select Sub-Ledger Name "/>
								<%-- <c:forEach var="subledger" items="${subledgerList}">
												<option value="${subledger.subledger_Id}">${subledger.subledger_name}</option>
											</c:forEach> --%>
						</form:select>
						</div>	
						<div class="col-md-1 control-label">
					...
					<div class="col-md-5">
						<form:select path="subledger_id2" class="logInput" id="select_cr_id">
							<form:option value="0" label="Select Sub-Ledger Name"/>
								<%-- <c:forEach var="subledger" items="${subledgerList}">
												<option value="${subledger.subledger_Id}">${subledger.subledger_name}</option>
											</c:forEach> --%>
	...
	function setDatePicker()
			...
			if('${mjDetails.dramount}'!=0)
		     {
				drDetails.push({
					...
					"drledgerId":'${mjDetails.subledgerdr.subledger_Id}',
					"dramountId":'${mjDetails.dramount}',
					"subledger_name":'${mjDetails.subledgerdr.subledger_name}',
					"debitId":debitId
	 ...
	 $("#dr_ledgerlist").val(JSON.stringify(drDetails)); 
					console.log($("#dr_ledgerlist").val());
					   ...
					   var markup = "<tr><td><a href = '#' onclick = 'deleteRow(this,"+'${mjDetails.subledgerdr.subledger_Id}'+","+'${mjDetails.dramount}'+","+drStatus+")'><img src='${deleteImg}' style = 'width: 20px;'/></a></td>"
				  		+"<td>"+'${mjDetails.subledgerdr.subledger_name}'+"</td>"+
		 ...
		 if('${mjDetails.cramount}'!=0)
		{
			 crDetails.push({
					...
					"crledgerId":'${mjDetails.subledgercr.subledger_Id}',
					"cramountId":'${mjDetails.cramount}',
					"subledger_namecr":'${mjDetails.subledgercr.subledger_name}',
					"creditId":creditId
		 ...
		 $("#cr_ledgerlist").val(JSON.stringify(crDetails)); 
					console.log($("#cr_ledgerlist").val());
					  ...
					  var markup = "<tr><td><a href = '#' onclick = 'deleteRowCr(this,"+'${mjDetails.subledgercr.subledger_Id}'+","+'${mjDetails.cramount}'+","+crStatus+")'><img src='${deleteImg}' style = 'width: 20px;'/></a></td>"
						+"<td>"+'${mjDetails.subledgercr.subledger_name}'+"</td>"+
						 "<td></td>"+ 
			...
			if(crledgerId == 0){
				...
				if(flag==false)
				{
					...
					for(i=0;i<subLedgerArray.length;i++)
		 			{
						subledger_id=subLedgerArray[i].id;
						if(crledgerId==subledger_id)
						{
					
						subledger_namecr = subLedgerArray[i].name;
						}
		 			}
					...
					for(i=0;i<subLedgerArray.length;i++)
		 			{
						...
						{
							subledger_name = subLedgerArray[i].name;
						}
		 			}
	...
	function getSubledgers(compId){
		
		$.ajax({
	        type: "POST",
	        url: "getSubledgers?id="+compId,
	        ...
	        success: function (data) {
	        	 ...
	        	 $('#select_dr_id').empty();
	        	 $('#select_dr_id').append('<option value="0">Select Sub-Ledger Name</option>');
	        	 $('#select_cr_id').empty();
	        	 $('#select_cr_id').append('<option value="0">Select Sub-Ledger Name</option>');
	        	 $.each(data, function (index, sub) {
	        		 
	        		 var tempArray = [];
					    tempArray["id"]=sub.subledger_Id;
					    tempArray["name"]=sub.subledger_name.replace('\'', '\'');
	        	 ...
	        	 subLedgerArray.sort(function(a, b) {
		 			...
		 			for(i=0;i<subLedgerArray.length;i++)
		 			{
		 				 $('#select_dr_id').append($('<option>', {
		 			    value: subLedgerArray[i].id,
		 			    text: subLedgerArray[i].name,
		 			})); 
		 				 
		 				 $('#select_cr_id').append($('<option>', {
				 			    value: subLedgerArray[i].id,
				 			    text: subLedgerArray[i].name,
				 			})); 
		 			}
