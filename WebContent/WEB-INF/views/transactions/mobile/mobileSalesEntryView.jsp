		...
		<div class="tab-content" style="padding:0;">
			<div class="tab-pane active" id="home">
				...
				<div class="table-responsive">
					<table class = "table">
						<tr>
							<td colspan='4' style='text-align:center'><strong> Financial Year:${entry.accountingYear.year_range}</strong></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td style="text-align: left;"><fmt:formatNumber type="number" minFractionDigits="2"	maxFractionDigits="2" value="${entry.cgst}" /></td>
						...
						<tr>			
							...
							<td style="text-align: left;">${entry.entrytype==1 ? "Local" : "Export" }</td>
						</tr>
						<tr>
				...
				<%-- <div class="col-md-12"  style = "text-align: center; margin:15px;">
						<c:if test="${entry.entry_status!=3}">
							<button class="fassetBtn" type="button" onclick = "DownloadPdf(${entry.sales_id})">   Download As Pdf       </button>
						</c:if>
					</div> --%>
			</div>
			...
			<%-- <div class="tab-pane" id="profile">
				<div class="row">
					...
					<div class="col-md-12">
						 <c:choose>
                  		...
                  		<p><b>Current Balance: <fmt:formatNumber type="number" value="${closingbalance}" /></b></p>
					</div>
				</div>
				<table class="table">
					<thead>
					...
					<tbody>
			            <tr>
							<td>${customername}</td>
							<td></td>
							<td><fmt:formatNumber type="number" minFractionDigits="2" maxFractionDigits="2" value="${entry.round_off}" /> Dr</td>
						</tr>
						<tr>
							<td>${entry.subledger.subledger_name}	</td>
							<td></td>
							<td><fmt:formatNumber type="number" minFractionDigits="2" maxFractionDigits="2" value="${entry.transaction_value}" /> Cr</td>
						</tr>
						<c:if test="${entry.tds_amount>0}">
						<tr>
							<td>TDS	</td>
							<td></td>
							<td><fmt:formatNumber type="number" minFractionDigits="2" maxFractionDigits="2" value="${entry.tds_amount}" /> Dr</td>
						</tr>
						</c:if>
						<c:if test="${entry.cgst!=0}">
							<tr>
								<td>CGST</td>
								<td></td>												
								<td><fmt:formatNumber type="number" minFractionDigits="2" maxFractionDigits="2" value="${entry.cgst}" /> Cr</td>
							</tr>				
						</c:if>
						<c:if test="${entry.sgst!=0}">
							<tr>
								<td>SGST</td>
								<td></td>
								<td><fmt:formatNumber type="number" minFractionDigits="2" maxFractionDigits="2" value="${entry.sgst}" /> Cr</td>											
							</tr>				
						</c:if>
						<c:if test="${entry.igst!=0}">
							<tr>
								<td>IGST</td>
								<td></td>
								<td><fmt:formatNumber type="number" minFractionDigits="2" maxFractionDigits="2" value="${entry.igst}" /> Cr</td>
							</tr>				
						</c:if>
						...
						<c:if test="${entry.total_vat!=0}">
							<tr>
								<td>VAT	</td>
								<td></td>
								<td><fmt:formatNumber type="number" minFractionDigits="2" maxFractionDigits="2" value="${entry.total_vat}" /> Cr</td>
							</tr>	
						</c:if>
						<c:if test="${entry.total_vatcst!=0}">
							<tr>
								<td>CST	</td>
								<td></td>
								<td><fmt:formatNumber type="number" minFractionDigits="2" maxFractionDigits="2" value="${entry.total_vatcst}" /> Cr</td>
							</tr>	
						</c:if>
						<c:if test="${entry.total_excise!=0}">
							<tr>
								<td>EXCISE</td>
								<td></td>
								<td><fmt:formatNumber type="number" minFractionDigits="2" maxFractionDigits="2" value="${entry.total_excise}" /> Cr</td>
							</tr>	
						</c:if>
			 ...
			 <div class="table-scroll" style="display:none;" id="tableDiv">
				<table id="Hiddentable" >
					...
					<tr>
						<c:choose>
						 	 <c:when test='${entry.sale_type==1}'>
						 	 	<td>Party A/c Name: Cash Sales</td>
						 	 	<c:set var="customername" value="Cash Sales"></c:set>
						 	 </c:when>
						 	 <c:when test='${entry.sale_type==2}'>
						 	 	<td>Party A/c Name: Card Sales - ${entry.bank.bank_name} - ${entry.bank.account_no}</td>
						 	 	<c:set var="customername" value="Card Sales- ${entry.bank.bank_name} - ${entry.bank.account_no}"></c:set>
						 	 </c:when>
						     <c:otherwise>
					...
					<tbody>
						...
						<c:if test="${entry.state_compansation_tax!=0}">
							<tr>
								<td>CESS</td>
								<td></td>
								<td><fmt:formatNumber type="number" minFractionDigits="2" maxFractionDigits="2" value="${entry.state_compansation_tax}" /> Cr</td>
							</tr>	
						</c:if>
						<c:if test="${entry.total_vat!=0}">
							<tr>
