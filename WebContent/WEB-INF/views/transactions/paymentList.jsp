...
<div class="breadcrumb">
	<h3>Payment</h3>
	<a href="homePage">Home</a> » <a href="paymentList">Payment</a>
</div>
<div class="col-md-12">
<c:choose>
...
<c:if test="${flagFailureList==true}">	
	...
	<c:if test="${importflag==false}">
		...
		<button  id="add" type="button" onclick = "checkImportPayment()">
				Add New Payment
		</button>
		</c:if>	
		...
		<button class="fassetBtn" type="button" onclick = "paymentFailure()">
					Failure List
		</button>
		</c:if> --%>
	</div>
					...
					<c:otherwise>
						...
						<td style="text-align: left;">
						   ...
						   <c:if test="${payment.accountingYear.year_id==year.accountingYear.year_id && year.yearEndingstatus==1}">		
							...
							<i  id='update_${payment.payment_id}' style="cursor: pointer;color:green;font-size: 15px;margin: 0px 3%;" 
							onclick = "editPayment('${payment.payment_id}')" class="acs-update fa fa-pencil" ></i>
							</c:if>
							 <i  id='delete_${payment.payment_id}' style="cursor: pointer;color:#ed1c24;font-size: 15px;margin: 0px 3%;" 
							  onclick = "deletePayment('${payment.payment_id}')" class="acs-delete fa fa-times" ></i>
							 </c:if>
						   </c:forEach>
						</td>	
					</c:otherwise>
