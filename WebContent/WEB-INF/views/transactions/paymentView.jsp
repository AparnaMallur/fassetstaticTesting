...
<div class="breadcrumb">
	<h3>Payment</h3>
	<a href="homePage">Home</a> » <a href="paymentList">Payment</a> » <a href="#">View</a>
</div>
		...
		<div class="tab-content">
			...
			<tr><td colspan='6' style='text-align:center'><strong>Financial Year:${payment.accountingYear.year_range}</strong></td></tr>
				...
				<tr>
					...
					<td style="text-align: left;">${payment.subLedger.subledger_name}
                   </td>
                   </c:when>
                    <c:otherwise> <td><strong>Bill No:</strong></td>
					<td style="text-align: left;">
				<c:choose>
					...
					<c:otherwise>
							...
							${payment.supplier_bill_no.supplier_bill_no}
									<c:set var="sup_bill_no" value="${payment.supplier_bill_no.supplier_bill_no}"></c:set>
													...
													<c:when test="${payment.advance_payment==true}">
										
									<c:set var="sup_bill_no" value="Advance"></c:set>
										Advance

											</c:when>
												<c:otherwise>
												 Opening Balance
											<c:set var="sup_bill_no" value="against Opening Balance"></c:set>
					...
					<td style="text-align: left;"> <fmt:parseDate value="${payment.date}" pattern="yyyy-MM-dd" var="parsedDate" type="date"/>
				...
				<tr>  			
				 ...
				 <tr>			
				<td style="display:none;" ><strong>GST Applicable:</strong></td>
					<td style="text-align: left; display:none;">${payment.gst_applied==true ? "Yes" : "No"}</td>			
				...
				<tr>
						<c:if test="${payment.tds_paid==true}">
							...
							<td style="text-align: left;"><fmt:formatNumber type="number" minFractionDigits="2"
									maxFractionDigits="2" value="${payment.tds_amount}" /></td>
				     	</c:if>
					        <td><strong>Status:</strong></td>
							<td style="text-align: left;">${payment.entry_status==3 ? "Disable" : "Enable"}</td>
							<td><strong>Closing Balance:</strong></td>
					<td style="text-align: left;">					
					<fmt:formatNumber type="number" value="${closingbalance}" /></td>
						...
						<c:choose>
							<c:when test="${payment.payment_type==1}">
								...
								<tr style='border: 1px solid #bbb'>
									...
									<tr>
                                <c:if test="${payment.other_remark!=null }"> 
								
                                  <td colspan="3">Remark:  ${payment.other_remark}</td>
                              </c:if>
                              <c:if test="${payment.other_remark==null }"> 
