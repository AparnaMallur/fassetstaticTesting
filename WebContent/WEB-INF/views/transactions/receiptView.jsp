...
<div class="fassetForm">
 ...
 --%>
						...
						<tr>
							<c:choose>
								<c:when test='${receipt.customer==null}'>
									...
									<td style="text-align: left;">${receipt.subLedger.subledger_name}
								...
								<c:otherwise>
									...
									<td style="text-align: left;"><c:choose>
										...
										<c:set var="cust_bill_no"
													value="${receipt.sales_bill_id.voucher_no}"></c:set>

											</c:when>
											<c:when test="${receipt.advance_payment==true}">
										
									<c:set var="cust_bill_no" value="Advance"></c:set>
										Advance
											...
											<c:otherwise>
										<c:set var="cust_bill_no"	value="againstOpeningBalnce"> </c:set>
													 Opening Balance
										</c:otherwise>
										</c:choose></td>
								</c:otherwise>
						...
						<tr>
							...
							<td><c:choose>
									<c:when test="${receipt.payment_type==1}">NA
								</c:when>
									<c:otherwise>
						...
						<tr>
							...
							<td style="text-align: left; display:none;">${receipt.gst_applied==1 ? "Yes" : "No"}</td>
							...
							<td style="text-align: left;"><fmt:formatNumber
									type="number" minFractionDigits="2" maxFractionDigits="2" value="${receipt.amount}" /></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td style="text-align: left;"><fmt:formatNumber
									type="number" value="${closingbalance}" /></td>
		...
		<c:if test="${receipt.entry_status!=3 || receipt.entry_status!=4}">
			<button class="fassetBtn" type="button"
				onclick="DownloadPdf(${receipt.receipt_id})">
			...
			<tr>
                                 <c:if test="${receipt.other_remark!=null }">
								
                                  <td colspan="3">Remark:  ${receipt.other_remark}</td>
                              </c:if>
                              <c:if test="${receipt.other_remark==null }">
