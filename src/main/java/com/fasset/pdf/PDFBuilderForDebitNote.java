...
public class PDFBuilderForDebitNote extends AbstractITextPdfView{
	...
	protected void buildPdfDocument(Map<String, Object> model, Document doc, PdfWriter writer,
		...
		response.setHeader("Content-Disposition", "attachment; filename=\"Debit Voucher.pdf\"");
		
		HttpSession session = request.getSession(true);
		User user = new User();
		user = (User)session.getAttribute("user");
		
		DebitNote debitNote = (DebitNote) model.get("debitNote");
		...
		supAddress=debitNote.getSupplier().getContact_name()+"\n"+debitNote.getSupplier().getCurrent_address()+"\nGSTIN/UIN:"+debitNote.getSupplier().getGst_no()
				+"\nState Name :"+debitNote.getSupplier().getState().getState_name()+"\nPin Code:"+debitNote.getSupplier().getPincode();
		
		invoiceNo="Debit Note No.\n"+debitNote.getVoucher_no();
		Date="Dated:\n"+debitNote.getDate().toString();
		...
		PdfPCell supAdd = new PdfPCell(new Phrase(supAddress, regularFont));
		supAdd.setColspan(4);
		supAdd.setRowspan(6);
		...
		if(debitNote.getDescription() != 2 && debitNote.getDescription() != 3) {
			...
			Double rate = 0.0;
			if(debitNote.getDebitDetails() != null) {
				
			}
			for(DebitDetails obj:debitNote.getDebitDetails()){			
			...
			total = total + debitNote.getCGST_head() + debitNote.getSGST_head()+ debitNote.getIGST_head()+debitNote.getSCT_head()+debitNote.getTotal_vat()+debitNote.getTotal_vatcst()+debitNote.getTotal_excise();
			...
			PdfPCell totalAmount = new PdfPCell(new Phrase("Amount Chargable(In Words):\n"+commonService.convertDoubleNumberToWord((Double) (Math.round(debitNote.getRound_off() * 100.0) / 100.0)),regularFont));
			...
			if (debitNote.getTotal_vat() > 0) {
				...
				for (GSTDetails gstDetail : gstDetails) {
					...
					cellRight.setPhrase(new Phrase(taxTotal.toString(), regularFont));
					taxTable.addCell(cellRight);
				}
			...
			} else if (debitNote.getIGST_head() > 0) {
				...
				totalTaxAmt.setVerticalAlignment(PdfPCell.ALIGN_CENTER);
				mainTable.addCell(totalTaxAmt);
			} else if (debitNote.getCGST_head() > 0 && debitNote.getSGST_head() > 0) {
				PdfPTable taxTable = new PdfPTable(9);
				taxTable.setWidthPercentage(100.0f);
				taxTable.setWidths(new float[] { 20.0f, 15.0f, 10.0f, 15.0f, 10.0f, 15.0f, 10.0f, 15.0f, 15.0f });
				taxTable.setSpacingBefore(10);
		...
		else
		{
			...
			mainTable.addCell(debitNote.getSubledger().getSubledger_name());
			}
			else
			{
				mainTable.addCell("");
			...
			mainTable.addCell("");
			mainTable.addCell("");
		    mainTable.addCell(debitNote.getRound_off().toString());
			...
			mainTable.addCell("");
			mainTable.addCell("");
			//cellRight.setPhrase(new Phrase(Double.toString((Double) (Math.round(debitNote.getRound_off() * 100.0) / 100.0)),boldFont));
			cellRight.setPhrase(new Phrase(calculate((Math.round(debitNote.getRound_off() * 100.0) / 100.0)).toString()));
		...
		String remarkString = "Remark:\n\nBeing debit note issued to "+debitNote.getSupplier().getCompany_name()+" for ";
		switch(debitNote.getDescription()) {
			...
			case 6: remarkString += "Finalization of Provisional Assessment";
					break;
			case 7: remarkString += debitNote.getRemark();
					break;
		}
