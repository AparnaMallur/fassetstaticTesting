...
public class PDFBuilderForExceptionReport2Pdf extends PDFBuilderForExceptionReport2PdfView{
	...
	public void buildPdfDocumentForQuotation(Map<String, Object> model, Document document, PdfWriter writer,
			HttpServletRequest request, HttpServletResponse response, List<ExceptionReport3Form> exceptionReport2FormsList,LocalDate fromdate,LocalDate todate,Company company) throws Exception {
		...
		Paragraph para = new Paragraph(new Phrase("Cash payment and receipt Unadjusted"));
		...
		cell.setPhrase(new Phrase("Aging in Days", font));
		table.addCell(cell);
		
		for (ExceptionReport3Form entry : exceptionReport2FormsList) 
		{
			...
			if(entry.getNumberOfDays()!=null) {
				table.addCell(entry.getNumberOfDays().toString());
			}
			else {
				table.addCell("");
			}
