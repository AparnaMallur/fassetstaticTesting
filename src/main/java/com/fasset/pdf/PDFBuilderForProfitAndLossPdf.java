...
public class PDFBuilderForProfitAndLossPdf extends PDFBuilderForProfitAndLossPdfView{
...
private CommonServiceImpl commonService = new CommonServiceImpl();
	...
	public void buildPdfDocumentForQuotation(Map<String, Object> model, Document document, PdfWriter writer,
			HttpServletRequest request, HttpServletResponse response, List<OpeningBalancesOfSubedgerForm> ListForOpeningbalancesOfsubledger,List<OpeningBalancesOfSubedgerForm> ListForOpeningbalancesbeforestartDate,LocalDate fromdate,LocalDate todate,Company company) throws Exception 
	{
		...
		if (subOpeningList.size() > 0) {
			...
			PdfPCell header1 = new PdfPCell(new Phrase("Profit and loss account debit side",boldFont));
			...
			PdfPCell header2 = new PdfPCell(new Phrase("Profit and loss account credit side",boldFont));
			...
			if(totalDebitSideAmount>totalCreditSideAmount)
			{
				creditTable.addCell(new Phrase("Net Loss", boldFont));
				creditTable.addCell("");
