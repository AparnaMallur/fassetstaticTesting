...
public class PDFBuilderForTrialBalanceReport extends AbstractITextPdfView {
	...
	protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter writer,
		...
		response.setHeader("Content-Disposition", "attachment; filename=\"TrialBalance Report.pdf\"");
		TrialBalanceReportForm form =(TrialBalanceReportForm) model.get("form");
		...
		//check error here
		//check list which called and balance showing which subledger wrong
		
		List<OpeningBalancesForm> supplierOpeningBalanceList= form.getSupplierList();
		List<OpeningBalancesForm> supplierOpeningBalanceBeforeStartDate = form.getSupplierList();
		List<OpeningBalancesForm> customerOpeningBalanceBeforeStartDate = form.getCustomerList();
		List<OpeningBalancesForm> customerOpeningBalanceList  = form.getCustomerList();
		List<OpeningBalancesForm>   bankOpeningBalanceBeforeStartDate =form.getBankList();
		List<OpeningBalancesForm>   bankOpeningBalanceList =form.getBankList();
		...
		if (form.getCompany()!= null) {
			...
			if (form.getCompany().getRegistration_no() != null) {
				if (form.getCompany().getCompany_statutory_type().getCompany_statutory_id().toString().equalsIgnoreCase("10")
						|| form.getCompany().getCompany_statutory_type().getCompany_statutory_id().toString()
								.equalsIgnoreCase("14")) {
